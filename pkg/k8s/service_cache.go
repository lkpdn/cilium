...
// all external endpoints if the service is marked as a global service. If the
// local endpoints are not present yet and no remote endpoint
func (s *ServiceCache) correlateEndpoints(id ServiceID) *Endpoints {
	...
	// If the no local endpoints is not present yet and not remote
	// endpoints are defined, return nil to indicate the service is not
	// ready yet.
	if !hasLocalEndpoints && len(endpoints.Backends) == 0 {
		return nil
	}
